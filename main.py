    ...
    def add_new_rectangles(self):
        try:
            ...
            if algorithm == BL_FILL:
                ...
                new_rects = pas.best_fit(self.canvas.width(), self.canvas.height(), self.rectangles, self.new_rectangles)
            elif algorithm == ANT_COLONY:
                new_rects = pas.ant_colony_optimization(self.canvas.width(), self.canvas.height(), self.rectangles, self.new_rectangles)
            else:
                raise ValueError("Неизвестный алгоритм")
        ...
        if ok1 and ok2:
            ...
            if ok3:
                ...
                if confirm == QMessageBox.StandardButton.Yes:
                    ...
                    index = next((i for i, rect in enumerate(self.new_rectangles) if rect.width == width and rect.height == height), None)
                    if index is not None:
                        self.new_rectangles[index] = Rectangle(new_width, new_height)
                        self.rect_list.takeItem(index)
                        self.rect_list.insertItem(index, f"Прямоугольник: {new_width}x{new_height}")
