    ...
    for y in range(canvas_height - height, -1, -1):
        for x in range(canvas_width - width + 1):
            ...
            if not any(rect.intersects(r) for r in rectangles):
                area = calculate_wasted_area_at_BLF(canvas_width, canvas_height, rectangles, rect)
                if area < best_area:
                    best_rect = rect
                    best_area = area
...
def calculate_wasted_area_at_BLF(canvas_width, canvas_height, rectangles, rect):
...
def calculate_wasted_area_at_bestFit(canvas_width, canvas_height, rectangles, rect):
    total_area = canvas_width * canvas_height
    used_area = sum([r.width() * r.height() for r in rectangles]) + rect.width() * rect.height()
    return total_area - used_area
...
def local_search(rectangles, new_rect):
    # Пример локального поиска для улучшения решения
    for i in range(len(rectangles)):
        if rectangles[i].intersects(new_rect):
            continue
        # Попробовать переместить новый прямоугольник
        temp_rect = QRect(rectangles[i].x(), rectangles[i].y(), new_rect.width(), new_rect.height())
        if not any(temp_rect.intersects(r) for r in rectangles):
            return temp_rect
    return new_rect


def hybrid_ant_colony_optimization(canvas_width, canvas_height, rectangles, width, height, num_ants=10,
                                   num_iterations=100, alpha=1.0, beta=2.0, evaporation_rate=0.5,
    ...
    for iteration in range(num_iterations):
        ...
        for rect in results:
            if rect:
                rect = local_search(rectangles, rect)  # Применение локального поиска
                area = calculate_wasted_area(canvas_width, canvas_height, rectangles, rect)
                if area < best_area:
                    best_rect = rect
                    best_area = area
